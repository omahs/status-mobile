(ns quo2.components.account-selector
  (:require
   [quo.theme :as theme]
...
(defn account-selector
  ...
  [{:keys [show-label? account-text transparent? label-text style]}]
    ...
    [rn/view {:style style}
     ...
     [rn/view {:style (account-container-row background-color)}
      [rn/view {:style account-emoji-container}
       [rn/image {:source (resources/get-image :default-account-avatar) :style account-emoji}]]
